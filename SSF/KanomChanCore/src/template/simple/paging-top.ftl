	    ...
	    <div class="col-md-12">
			<div class="inline">
				...
				<script>
					function rowsPerPage_Top_${parameters.id}_select(){
						$('#currentPage').val('1');
						$('#rowsPerPage').val($('#rowsPerPage_Top_${parameters.id}').val());
			...
			<ul class="pagination pull-right inline">
				...
				<#if parameters.currentPage??>
					<@s.hidden id="currentPage" name="pagingBean.currentPage" value="${parameters.currentPage}"></@s.hidden>
					<#else>
						<@s.hidden id="currentPage" name="pagingBean.currentPage"></@s.hidden>
				</#if>
				<#if parameters.rowsPerPage??>
					<@s.hidden id="rowsPerPage" name="pagingBean.rowsPerPage" value="${parameters.rowsPerPage}"></@s.hidden>
					<#else>
						<@s.hidden id="rowsPerPage" name="pagingBean.rowsPerPage"></@s.hidden>
			 	</#if>
			 	<#if parameters.isAjax??>
<#-- -------------------------------------------- -->
			 		<div class="${parameters.id}_paging_clone" style="display: none;">
			 			<#if parameters.currentPage == 1>
							<li><a class="disabled">&laquo;</a></li>
							<#else>
								<li><a href="#" onclick="$('#currentPage').val('${parameters.previousPage?html}');${parameters.id}_goToPage();">&laquo;</a></li>
