	    ...
	    <div class="col-md-12">
			...
			<ul id="${parameters.id}_pagination_top" class="pagination pull-right inline ${parameters.id}_pagination">
					...
					<@s.hidden id="rowsPerPage_${parameters.id}" name="pagingBean.rowsPerPage" value="${parameters.rowsPerPage?default(parameters.defaultShowPerPage)}"></@s.hidden>
			 		<script>
			 			function ${parameters.id}_reset_paging(){
							...
							$('.${parameters.id}_label').append('<label class="control-label"><@s.text name="COMMON_PAGING_SHOWING"></@s.text> ${(parameters.currentPage * parameters.rowsPerPage - parameters.rowsPerPage + 1)} <@s.text name="COMMON_PAGING_TO"></@s.text> <@s.if test="%{${parameters.currentPage} == ${parameters.pageCount}}"> ${parameters.totalRows}</@s.if><@s.else> ${parameters.currentPage * parameters.rowsPerPage}</@s.else> <@s.text name="COMMON_PAGING_FROM_TOTAL"></@s.text> ${parameters.totalRows} <@s.text name="COMMON_PAGING_ROWS"></@s.text></label>');
				...
				<#else>
					...
					<#if parameters.displayPage[0] gt 1 >
						<li><a onclick="$('#currentPage_${parameters.id}').val('1');${parameters.id}_goToPage();">1</a></li>
					</#if>
					<#if parameters.displayPage[0] gt 2>
						<li><a onclick="$('#currentPage_${parameters.id}').val('2');${parameters.id}_goToPage();">2</a></li>
					</#if>
					<#if parameters.displayPage[0] gt 3>
						<li><a class="paging-hidden">...</a></li>
					</#if>
					<@s.iterator var="count" value="parameters.displayPage">
						<@s.if test='%{${parameters.currentPage?string["0"]} == #count}'>
							<li><span>${parameters.currentPage?string["0"]}</span></li>
						</@s.if>
						<@s.else>
							<li><a onclick="$('#currentPage_${parameters.id}').val('<@s.property value="%{#count}"/>');${parameters.id}_goToPage();"><@s.property value="%{#count}"/></a></li>
						</@s.else>
					</@s.iterator>
					<#if parameters.displayPage.get(parameters.displayPage.size()-1) lt parameters.pageCount-2 >
						<li><a class="paging-hidden">...</a></li>
					</#if>
					<#if parameters.displayPage.get(parameters.displayPage.size()-1) lt parameters.pageCount-1>
						<li><a onclick="$('#currentPage_${parameters.id}').val('${(parameters.pageCount-1)?string["0"]}');${parameters.id}_goToPage();">${(parameters.pageCount-1)?string["0"]}</a></li>
					</#if>
					<#if parameters.displayPage.get(parameters.displayPage.size()-1) lt parameters.pageCount>
						<li><a onclick="$('#currentPage_${parameters.id}').val('<${parameters.pageCount?string["0"]}');${parameters.id}_goToPage();">${parameters.pageCount?string["0"]}</a></li>
					</#if>
